...
const getDashboardRedirect = (role) => {
  ...
  if (normalized === 'admin' || normalized === 'institution_admin') return '/admin';
  return '/student';
};
...
const Login = () => {
  ...
  const handleMetaMaskLogin = async () => {
    ...
    try {
      ...
      const nonceResponse = await authAPI.getMetamaskLoginNonce({ walletAddress });
      const { message } = nonceResponse.data;
      const signature = await signer.signMessage(message);

      const loggedUser = await loginWithMetaMask(walletAddress, signature);
      navigate(getDashboardRedirect(loggedUser.role));
    } catch (error) {
  ...
  const handleRegisterSubmit = async (event) => {
    ...
    setIsLoading(true);
    try {
      await superAdminAPI.registerRequest({
        name: regName,
        code: regCode.toUpperCase(),
        email: regEmail,
      });
      toast.success('Gửi đơn yêu cầu thành công! Đang chờ duyệt.');
          ...
          {!isRegistering ? (
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                >
                  <Plus size={18} /> Đăng ký trường/học viện mới
                </Button>
              </div>
            ...
            // Form yêu cầu đăng ký trường học mới
            <>
              <div className="text-center sm:text-left">
                <Badge variant="primary" className="mb-4">
                  <BookOpen size={14} />
                ...
                <h2 className="text-3xl font-extrabold tracking-normal text-slate-950">Đăng ký thành viên</h2>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Gửi yêu cầu đăng ký cho Super Admin. Khi được duyệt, hợp đồng thông minh riêng sẽ tự động được khởi tạo.
                </p>
              </div>

              <form onSubmit={handleRegisterSubmit} className="mt-8 space-y-5">
                <div>
                  <label className="form-label">Tên trường / Học viện</label>
                  <input
                    type="text"
                    value={regName}
                ...
                <div>
                  <label className="form-label">Mã viết tắt trường (Ví dụ: UIT, HUST)</label>
                  <input
                    type="text"
                    value={regCode}
                ...
                <div>
                  <label className="form-label">Email liên hệ nhận tài khoản quản trị</label>
                  <input
                    type="email"
                    value={regEmail}
                    onChange={(event) => setRegEmail(event.target.value)}
                    className="form-control"
                    placeholder="phongdaotao@university.edu"
                    required
                  />
                </div>
